  ...
  <div>
    <div class="page-header page-header-small">
      ...
      <div class="content-center">
        <div class="container">
          <h1 class="title">Manufacturing Process Steps</h1>
        </div>
      </div>
    </div>
  ...
  created() {
    Axios.get("/Etapes/" + this.id).then(
      (response) => (
        ...
        (this.form.name = this.info["etap_nom"]),
        (this.form.desc = this.info["etap_description"])
      )
    );
  },
  ...
  methods: {
    async submit() {
      if (!this.form.name || !this.form.desc) {
        ...
        Axios.put("/Etapes/update/" + this.id, {
